...
const generateAPIKey = async (userId) => {
    ...
    const encryptedKey = await bcrypt_1.default.hash(key, salt);
    ...
    await user_model_1.UserModel.findByIdAndUpdate(userId, { apiKey: encryptedKey, apiKeyExpiration: expirationTime }, { new: true });
    // Fetch the updated API key information for the user
    const foundUser = await user_model_1.UserModel.findById(userId).select('apiKey apiKeyExpiration').lean().exec();
    if (!foundUser) {
