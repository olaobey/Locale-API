...
describe('Signup Endpoint', () => {
    ...
    it('should create a new user', async () => {
        ...
        const response = await (0, supertest_1.default)(app_1.default).post('/api/v1/auth/register').send(userData);
        expect(response.status).toBe(200);
        expect(response.body.success).toBe(true);
        expect(response.body.message).toBe('SignUp successfully');
        expect(response.body.newUser).toBeDefined();
        // Check if the user is saved in the database
        const savedUser = await user_model_1.UserModel.findOne({ email: userData.email }).exec();
