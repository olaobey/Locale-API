...
const signup = async (req, res, next) => {
    ...
    try {
        ...
        const existingUser = (await user_model_1.UserModel.findOne({ email }).exec());
...
const signin = async (req, res, next) => {
    try {
        ...
        const foundUser = (await user_model_1.UserModel.findOne({ username }).select('+password').exec());
        if (!foundUser) {
        ...
        const { maskedKey, apiKeyInfo } = await (0, apiKey_service_1.generateAPIKey)(foundUser._id);
...
const logout = async (req, res, next) => {
    try {
        ...
        const apiKey = await user_model_1.UserModel.findById(userId).exec();
        if (!apiKey) {
